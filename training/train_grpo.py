"""TRL-backed policy training entrypoint built on live environment rollouts."""
...
except ImportError:  # pragma: no cover - exercised in Colab, not local smoke tests
    Dataset = None
    LoraConfig = None
...
def build_prompt(observation: dict[str, Any]) -> str:
    return (
        ...
        f"Visible alerts: {observation.get('visible_alerts', [])}\n"
            ...
            callback.log(
                {
                    ...
                    "success": float(observation.get("data", {}).get("benchmark_score", 0.0)) >= 0.8,
                    "benchmark_score": float(observation.get("data", {}).get("benchmark_score", 0.0)),
                }
            )
